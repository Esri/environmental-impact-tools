...
#These fields will be summerized and localized
# so a value of 1000 will show as 1,000 for example
SUM_FIELDS = [AREA_FIELD, LEN_FIELD, COUNT_FIELD, PERCENT_FIELD]

#Key values that will be added by the analysis tools
# The rows with these values will be grouped and shown in seperate tables in the report
SPLIT_VAL_BUFFER = 'Buffer'
SPLIT_VAL_AOI = 'AOI'

#Values that will be appended or shown in the report based on various conditions
...
#Margins to be applied to the values to allow for a small gap between the value 
# and the lines for the cell borders
Y_MARGIN = .025
X_MARGIN = .06

#This will format float values so no more than 2 digits will be seen after a decimal
NUM_DIGITS_2 = '{0:.2f}'
...
class MockField:
    #Handle fields that generated by the report  in a similar way 
    # as fields that exist in the input dataset
...
class Table:
    ##Report Table##
    ##This will review the provided values and determine 'cell' size needed
    ## to both fit the value and stay within the defined content display area
    def __init__(self, title, rows, fields):
        global _msg
        _msg = 'initializing table: ' + title
        self.rows = rows
        self.fields = fields
        self.title = title
        ...
        #key elements expected to be in the layout template
        ...
        #used when a table overflows from one page to the next
        ...
        if ANALYSIS_PROP_FIELD in self.field_names and ANALYSIS_DESC_FIELD in self.field_names:
            #specific elements are used for this table type to allow for 
            # difference between this summary data and standard table results 
            self.is_analysis_table = True

    def calc_widths(self):
        #Determine the required field widths based on the font size specified
        # by the template element and the length of the actual field name or value
        global _msg
        _msg = 'calculating row widths.'
        self.auto_adjust = []
        self.field_name_lengths = []
        self.field_widths = []
        ...
        #when the base row width exceeds the potential display area
        # we need to understand what rows have values that would need to be wrapped
        #As well as determine how wide each field would need to be to make them all fit
    ...
    def get_max_vals(self, rows):
        ...
        # elements text property we check a few things here.
        #We look at both the number of characters and evaluate if a row has a significant
        # mix of upper and lower case characters to help ensure that the max value
        # we will use as the template is fairly representitive of other values in the dataset.
        global _msg
        _msg = 'determining maximum value length.'
        vals = []
        indexes = []
        fr = True
        all_lower = {}
    ...
    def adjust_row_widths(self):
        #When the base row width exceeds the potential display area we need to 
        # determine what fields can and should be adjusted.
        global _msg
        _msg = 'adjusting row width to fit display area.'
        auto_adjust = []
    ...
    def calc_num_chars(self, fit_width, v, column_index):
        #examine how many chacters will fit in the potential field width
        #The determined value is used for wrapping of the charcters.
        global _msg
        _msg = 'adjusting chracters to fit display area.'
        elm = self.field_value
    ...
    def calc_heights(self):
        #Determine how tall a 'cell' would need to be in order to fit max value
        global _msg
        _msg = 'determining cell height.'
    ...
    def init_elements(self, elements, layout_type):
        global _msg
        _msg = 'initializing layout elements.'
        #Get key elelments
        if self.is_analysis_table:
...
class Report:
    ...
    def init_overflow_layout(self, template):
        global _msg
        _msg = 'initializing overflow layout.'
        pagx = None
        if template in ['', '#', ' ', None]:
    ...
    def find_elements(self, type):
        global _msg
        _msg = 'locating ' + str(type) + ' layout elements.'
        ...
        if len(layouts) > 0:
            ...
            global msg
            msg = []
            msg.append("Cannot locate Layout: " + layout_name)
            sys.exit()

    def update_layouts(self):
        global _msg
        _msg = 'updating layouts.'
        self.set_layout('map', self.map_pagx)
        x = 0
        t=None
        first_overflow = True
        for table in self.tables:
            _msg = 'updating ' + table.title + ' layout.'
    ...
    def update_overflow_table(self, table, overflow_table, first_overflow):
        global _msg
        _msg = 'updating overflow table: ' + table.title
    ...
    def set_layout(self, layout_type, pagx):
        ...
        if len(layouts) > 0:
            ...
            global msg
            msg = []
            msg.append("Cannot find layout:" + layout_name)
            sys.exit()
            self.remaining_height = None

    def set_element_props(self):
        global _msg
        _msg = 'setting element properties.'
    ...
    def append_pages(self):
        ...
        try:
            ...
            if len(self.pdf_paths) > 0:
                ...
                global msg
                msg = []
                msg.append('Unable to export report')
                sys.exit()
        for pdf in self.pdf_paths:
...
def main():
    ...
    try:
        ...
        for table in tables:
            ...
            arcpy.AddMessage("Processing table: " + str(table_title))       
            ...
            if len(fields) < 1:
                arcpy.AddError("Report table must include one or more field.")
                arcpy.AddWarning("Unable to process table: " + str(table_title))
            else:
                try:
                    _msg = 'checking domains.'
                    workspace = desc.path
                    ...
                    for r in cur:
                        ...
                        for v in r:
                            ...
                            _v = v
                            if d:
                                if len(d) > 0 and f.domain != '':
                                    for _d in d:
                                        if f.domain in _d.keys():
                                            if v in _d[f.domain]:
                                                _v = _d[f.domain][v]
                            tr.append(str(test_trim(_v)).replace('\n',' '))
                            x += 1
                        test_rows.append(tr)
                    _msg = 'adding table to the report.'
                    report.add_table(table_title, test_rows, fields)
                except Exception:
                    arcpy.AddWarning("Unable to process table: " + str(table_title))
                    if _msg != "":
                        arcpy.AddError("Error {0}".format(_msg))
        ...
        if len(msg) > 0:
            arcpy.AddError("Error: {0} (in file {1}, {2})".format(msg[0], filename, line))
            if len(msg) > 1:
                x = 1
                while x < len(msg):
                    arcpy.AddError(msg[x])
                    x += 1
        else:
            arcpy.AddError("Error in file {0}, {1}, {2})".format(filename, line, synerror))
    except:
        line, filename, synerror = trace()
        if _msg != "":
            arcpy.AddError("Error {0}".format(_msg))
