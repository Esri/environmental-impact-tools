...
#Elements should be anchored at upper left 
# and have the appropriate names
#On import of custom pagx we need to pass through all key elements and 
#ensure that the anchor is set appropriately

#look into delete layout
...
# table title needs to be set at X=0 and then vertically centered
...
class Table:
    def __init__(self, title, rows, fields):
        ...
        self.report_subtitle = None
        self.scale_bar = None
        self.logo = None
        self.page_number = None
        self.report_title_footer = None
    ...
    def calc_widths(self):
        ...
        for f in self.fields:
            ...
            field_name_lengths.append(self.field_name.elementWidth + m)      
        ...
        for row in self.rows:
            ...
            for v in row:
                ...
                if len(self.column_widths) == len(field_name_lengths):
                    length = self.column_widths[x]
                    update = True
                self.field_value.text = v
                potential_length = self.field_value.elementWidth + m
                if potential_length > length:
                    length = potential_length
                if update:
                    self.column_widths[x] = length
                else:
                    self.column_widths.append(length)
                x+=1
        self.row_width = sum(self.column_widths)

        if self.row_width > self.content_display.elementWidth:
            auto_adjust = self.adjust_row_widths(field_name_lengths)        
    ...
    def adjust_row_widths(self, field_name_lengths):
        ...
        if overflow_width > display_width:
            print("TODO...do we deal with this and will wrapping be enough?")
        if sum(field_name_lengths) > display_width:
            print("TODO...do we deal with truncating of field names?")

        large_column_widths = []
        idx = 0
        for column_width in self.column_widths:
            if column_width > display_width / len(self.column_widths):
                large_column_widths.append(column_width)
            else:
                large_column_widths.append(0)
                display_width -= self.column_widths[idx]
            idx += 1

        idx = 0
        sum_widths = sum(large_column_widths)
        pos_widths = [x for x in large_column_widths if x != 0 ]
        for w in large_column_widths:
            if float(w)/float(sum_widths) > 0:
                self.column_widths[idx] = display_width * (1/len(pos_widths))
    ...
    def calc_heights(self):
        ...
        if len(self.auto_adjust) > 0:
            ...
            for column_index in self.auto_adjust:
                col_width = self.column_widths[column_index]
                ##width hack TODO has to be a better way
                #self.field_value.text = "w"
                #while self.field_value.elementWidth < col_width:
                #    self.field_value.text += "w"
                row_x = 0
                for row in self.rows:
                    s_i = 0
                    self.field_value.text = ""
                    while self.field_value.elementWidth < (col_width - MARGIN *2):
                        if len(str(row[column_index])) > s_i:
                            self.field_value.text += str(row[column_index])[s_i]
                            s_i += 1
                        else:
                            break
                    wrapped_val = textwrap.wrap(str(row[column_index]), (len(self.field_value.text)))

                    wrapped_height = (len(wrapped_val) * self.row_height) + (len(wrapped_val) - 1) * MARGIN
                    if wrapped_height > row_heights[row_x]:
                        row_heights[row_x] = wrapped_height
                    row_x += 1
        else:
    ...
    def init_elements(self, elements, layout_type):
        ...
        self.logo = elements['Logo']
        self.page_number = elements['PageNumber']
        self.report_title_footer = elements['ReportTitleFooter']

        #Map only elements
        if layout_type == 'map':
            self.report_subtitle = elements['ReportSubTitle']
            self.scale_bar = elements['Scale Bar']
            self.report_type = elements['ReportType']
            self.report_title = elements['ReportTitle']
            self.map_frame = elements['Map Map Frame']
...
class Report:
    def __init__(self, report_title, sub_title, logo, map, scale_unit, report_type, map_template, overflow_template):
        ...
        #TODO...should just use the name of the 
        ...
        self.init_layouts(map_template, overflow_template, False)
    ...
    def init_layouts(self, map_template, overflow_template, is_overflow):
        if not is_overflow:
            map_pagx = None
            if map_template in ['', '#', ' ', None]:
                ...
                map_pagx = self.write_pagx(map_layout_json, self.map_layout_name)
            else:
                map_pagx = map_template
            self.map_pagx = map_pagx
            self.aprx.importDocument(self.map_pagx)

        overflow_pagx = None
        if overflow_template in ['', '#', ' ', None]:
            ...
            overflow_pagx = self.write_pagx(overflow_layout_json, self.overflow_layout_name)
        else:
            overflow_pagx = overflow_template
        self.overflow_pagx = overflow_pagx
    ...
    def find_elements(self, type):
        ...
        if len(layouts) > 0:
            ...
            for elm_name in element_names:
                ...
                if len(elms) > 0:
                    arcpy.AddMessage("Found: " + elm_name)
                    elm = elms[0]
                    out_elements[elm.name] = elm
                else:
                    arcpy.AddMessage("Cannot find: " + elm_name)
            return out_elements
        else:
            arcpy.AddMessage("Cannot locate Layout: " + layout_name)
    ...
    def update_layouts(self):
        ...
        for table in self.tables:
            if table.is_overflow: #does this happen again before the next overflow
                if not a:
            ...
            #self.remaining_height = table.remaining_height
            x += 1
            if overflow:
    ...
    def set_layout(self, layout_type, pagx):
        ...
        if len(layouts) > 0:
            ...
            arcpy.AddMessage("Cannot find overflow layout template.")
            sys.exit()
            self.remaining_height = None
    ...
    def add_values(self, table):
        ...
        for row in table.rows:
            for v in row:
                ...
                if len(table.auto_adjust) > 0:
                    for i in table.auto_adjust:
                        if x == i:
                            #width hack
                            si = 0
                            elm.text = ""
                            while elm.elementWidth < (table.column_widths[x] - (MARGIN * 2)):
                                if len(str(v)) > si:
                                    elm.text += str(v)[si]
                                    si += 1
                                else:
                                    break
                            v = textwrap.fill(str(v), (len(elm.text)))
                elm.text = v
                elm.elementPositionX = self.cur_x + (MARGIN / 2)
                elm.elementPositionY = self.cur_y - (MARGIN / 4)
                self.cur_x += table.column_widths[x]
...
def main():
    ...
    tables = arcpy.GetParameterAsText(3).split(';')        #required multivalue parameter for input tables
    map = arcpy.GetParameter(4)                      #required parameter for the map 
    ...
    for table in tables:
        ...
        if desc.dataType == "FeatureLayer" or desc.dataType == "TableView":
            ...
            test_fields = [f.name for f in desc.fields if f.type not in ['Geometry'] and
                           fi.getVisible(fi.getFieldByName(f.name) == 'VISIBLE')]
        else:
            test_fields = [f.name for f in desc.fields if f.type not in ['Geometry']]
    ...
    arcpy.AddMessage("PDF Generated: " + pdf)
    os.startfile(pdf)

def test():
    report_title = "Generic Resource Assignment Analysis1234"
    map_report_template = None
    overflow_report_template = None
    ##Test Table1
    report_title = "a"             #required parameter for report title
    sub_title = "as"                #optional parameter for sub-title
    ...
    tables = r"C:\Temp\New File Geodatabase.gdb\LongFieldNames".split(';')        #required multivalue parameter for input tables
    map = None                      #required parameter for the map 
    scale_unit = None               #required scale unit with default set
    report_type = "A"              #optional report type eg. ...
    ...
    #out_folder = arcpy.GetParameterAsText(10)    

    table_title = "Eagle Nesting Locations within Buffer"
    test_rows = [["HL054", "Hillsborough dfsdfsdf sdf sdf sdfgdfg sdg fdsg sdfg fdsgfd sdsfg dsfg dsfg dsfgdsfg  dfgdfgdfsgdfgs", "256.3553791"],
    ...
    test_fields = ["NestID", "C", "D"]
    table_title2 = "Eagle Nesting Locations within Buffer2"
    table_title3 = "Eagle Nesting Locations within Buffer3"
    #create the ReportData and add the table
    
    report = Report(report_title, "A", logo, "", "Meter", "report_type", map_report_template, overflow_report_template)
    report.add_table(table_title, test_rows, test_fields)
    report.add_table(table_title2, test_rows, test_fields)
    report.add_table(table_title3, test_rows, test_fields)
